...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
	...
	for (G4GDMLAuxMapType::const_iterator iter = auxmap->begin(); iter != auxmap->end(); iter++) 
	{
		...
		G4double opacity = 1.0; // Default opacity value
		...
		for (G4GDMLAuxListType::const_iterator vit = (*iter).second.begin(); vit != (*iter).second.end(); vit++)
		{
			...
			// Check if the auxiliary information is for opacity
			else if ((*vit).type == "opacity") 
			{
				// Parse the opacity string into a single float value
				std::istringstream iss((*vit).value);
				if (!(iss >> opacity)) 
				{
					G4cerr << "Error parsing opacity string: " << (*vit).value << G4endl;
				}
			}
		}

		// Create a G4Colour object with the parsed values
		G4Colour colour(r, g, b, opacity);
